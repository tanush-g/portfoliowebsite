...
const useSkater = (containerRef) => {
  ...
  const minFrameTimeRef = useRef(1000 / 120) // Minimum time between frames for max 120 FPS
  ...
  useEffect(() => {
    ...
    // Capture scene reference to use in cleanup
    ...
    // Animation loop with FPS capping to max 120 FPS
    const animate = (currentTime) => {
      ...
      // Only render if enough time has passed (cap at 120 FPS)
      if (elapsed >= minFrameTimeRef.current) {
        ...
        if (frameCountRef.current <= 120) {
          ...
          const rotSpeed = -easeOutCirc(frameCountRef.current / 120) * Math.PI * 20
          camera.position.y = 5
    ...
    return () => {
      ...
      if (document.body.contains(container) === false) {
        cleanupThreeJS(scene, rendererInstance, controls, animationFrameId)
